use crate::{
    ast::{Binary, BinaryOp, Bool, Call, First, Function, If, Let, Print, Second, Term, Tuple},
    parser::Var,
};
...
pub fn ast_to_hir(ast: Term) -> Vec<Expr> {
    ...
    loop {
        let (expr, next) = match ast {
            ...
            Term::Var(Var { text, .. }) => (Expr::Var { name: text }, None),
        };
        result.push(expr);
        match next {
            Some(next) => {
...
fn binop_hir(lhs: Term, rhs: Term, op: BinaryOp) -> Expr {
